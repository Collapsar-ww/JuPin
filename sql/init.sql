...
CREATE TABLE `order` (
  ...
  `idempotent_key`  VARCHAR(128)  DEFAULT NULL COMMENT '创建订单幂等Key',
  `user_id`         BIGINT        NOT NULL,
  `pool_id`         BIGINT        NOT NULL,
  ...
  `pay_request_no`  VARCHAR(64)   DEFAULT NULL COMMENT '支付请求幂等号',
  `callback_request_no` VARCHAR(64) DEFAULT NULL COMMENT '支付回调幂等号',
  `expire_time`     DATETIME      DEFAULT NULL COMMENT '订单支付截止时间',
  ...
  UNIQUE KEY `uk_user_idempotent_key` (`user_id`, `idempotent_key`),
  UNIQUE KEY `uk_channel_txn_id` (`channel_txn_id`),
  KEY `idx_user` (`user_id`),
