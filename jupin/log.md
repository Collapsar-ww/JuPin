...
### 本轮操作：修复订单创建幂等预查后的重复类型订单异常

#### 1. 问题现象

第三版 A/B 梯度压测进入 `idempotent` 场景后，优化分支在 50 并发同一 `idempotentKey` 创建订单时仍偶发业务失败：

- HTTP 状态为 200；
- 业务 `code=500`；
- 响应消息为“已创建过该类型的订单”；
- 同轮最终仍只落 1 条或少量订单，但部分重复请求没有幂等返回已有订单。

#### 2. 根因

`OrderServiceImpl.create()` 的流程是：

1. 先按 `user_id + idempotent_key` 查询已有订单；
2. 再检查 `user_id + pool_id + type` 是否已有 PENDING/PAID 订单；
3. 如果已有同类型订单，直接抛出“已创建过该类型的订单”。

高并发下，部分请求在第一步预查时还看不到已有订单，但在第二步检查时，另一个并发请求已经提交成功，于是命中同类型订单检查并抛业务异常。这个分支没有走幂等返回。

#### 3. 修复内容

- 将同类型订单检查从 `selectCount + throw` 改为查询已有订单并直接返回。
- 这样无论重复请求通过 `idempotent_key` 命中，还是通过 `user_id + pool_id + type` 命中，都统一幂等返回已有订单。
- 保留 `DuplicateKeyException` 后的短重试回查，继续处理唯一索引并发竞争窗口。
